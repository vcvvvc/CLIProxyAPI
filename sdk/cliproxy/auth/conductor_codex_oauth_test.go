...
func TestSelectCodexOAuthAuthSkipsAPIKeyAndRuntimeFreeze(t *testing.T) {
	...
	auths := []*Auth{
		...
		{ID: "b-frozen-oauth", Provider: "codex", Metadata: map[string]any{"email": "frozen@example.com"}, Unavailable: true, NextRetryAfter: until, Quota: QuotaState{Exceeded: true, Reason: successFreezeReason, NextRecoverAt: until}},
...
func TestSelectCodexOAuthAuthHomeMaintainsCountAndProvider(t *testing.T) {
	...
	store := &codexOAuthFreezeStore{freezes: []RuntimeFreezeState{{AuthID: "frozen", Reason: successFreezeReason, NextRecoverAt: until}}}
...
// What：验证 GPT-5.5 成功请求在第 50 次触发 limit_50 并从后续选择中排除。
// Why：Search 复用 Responses 执行链，必须证明原有计数、冻结和 sidecar 持久化仍闭环。
func TestGPT55SuccessLimitFreezeExcludesAuth(t *testing.T) {
	...
	for i := int64(0); i < successFreezeThreshold-1; i++ {
	...
	if before.Success != 49 || before.successFreezeCount != 49 || before.Unavailable {
	...
	if frozen.Success != 50 || frozen.successFreezeCount != 0 || !frozen.Unavailable || frozen.Quota.Reason != successFreezeReason {
	...
	if len(store.saved) != 1 || store.saved[0].AuthID != "a-primary" || store.saved[0].Reason != successFreezeReason {
